...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    if($tipo === 'docencia') {
        ...
        if($guardia){
            // Borrar origen y destino
            $pdo->prepare("DELETE FROM guardias WHERE profesor_id=? AND tramo_id=?")->execute([$profesor_id, $tramo_origen]);
            $pdo->prepare("DELETE FROM guardias WHERE profesor_id=? AND tramo_id=?")->execute([$profesor_id, $tramo_destino]);
            // Insertar en destino
            $pdo->prepare("INSERT INTO guardias (profesor_id,tramo_id,dia_semana) VALUES (?,?,?)")
                ->execute([$profesor_id,$tramo_destino,$guardia['dia_semana']]);
        }
    }
